...
func (as *AdService) Get(ctx context.Context, id string) (*Ad, error) {
	...
	err := as.c.GetJSON(ctx, fb.NewRoute(Version, "/%s", id).Fields("id", "creative", "name", "account_id", "adset_id",
		"adset{id,daily_budget,name,start_time,end_time,status,bid_strategy,targeting{age_min,age_max,publisher_platforms,facebook_positions,instagram_positions,audience_network_positions,messenger_positions,geo_locations,genders,custom_audiences,excluded_custom_audiences,flexible_spec,exclusions}}",
...
// ListOfCampaign returns all ads of a campaign.
func (as *AdService) ListOfCampaign(campaignID string) *AdListCall {
	return &AdListCall{
		RouteBuilder: fb.NewRoute(Version, "/%s/ads", campaignID).Fields("id", "adset{id}", "adcreatives{id}").Limit(1000),
		c:            as.c,
	}
}
